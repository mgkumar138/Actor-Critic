...
Advantage = current reward + y*future reward - value of current state
train on batch 

Implementation of online actor-critic?
'''
...
class ACagent:
    ...
    def act(self, state):
        action_dist, _ = self.model(tf.convert_to_tensor(state[None, :], dtype=tf.float32))
    ...
    def compute_loss(self, memory, discounted_rewards):
        ...
        neg_log_prob = tf.nn.softmax_cross_entropy_with_logits_v2(logits=logit, labels=np.array(memory.actions)[None,:]) # Nx1 tensor

        policy_loss = neg_log_prob * tf.stop_gradient(advantage) # becomes NxN tensor

        entropy = tf.nn.softmax_cross_entropy_with_logits_v2(logits=logit, labels=tf.nn.softmax(logit))

        policy_loss_minent = policy_loss - 0.01 * entropy

        # custom loss
        cross_entropy = memory.actions * tf.log(tf.nn.softmax(logit))
        neg_log_prob2 = -tf.reduce_sum(cross_entropy, 1)

        policy_loss2 = neg_log_prob * advantage # NxN tensor
...
def main(episodes):
    ...
    for e in range(episodes):
        ...
        for t in range(time):
            ...
            onehot_action = to_categorical(action, agent.action_size)

            mem.store(state, onehot_action, reward)
        ...
        meanR = np.mean(tot_r)
        maxR = np.amax(tot_r)
        print('Episode {} of {}, last for {}s, Mean Reward {}, Max Reward {}, Training Loss {}'.format(e, episodes,
                                                                                                       tot_r[-1], meanR,
                                                                                                       maxR, tot_loss[-1]))
...
if __name__ == "__main__":
    ...
    plt.figure(4)
    plt.plot(list(range(1, bootstrap + 1)), strap_run)
    plt.title('A2C CartPole, Average Runtime {}s'.format(np.round(np.mean(strap_run),3)))
